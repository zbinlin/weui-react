...
export default class App extends Component {
    ...
    actionSheetOnClick(callback, beforeClose) {
        callback().then(idx => {
            alert(`You click ${idx}`);
            return beforeClose();
        }).then(() => {
    ...
    render() {
        ...
        let toast = this.state.toast ? <Toast>已完成</Toast> : null;
        let loading = this.state.loading ? <Loading>数据加载中</Loading> : null;
        let actionSheet = this.state.actionSheet ? <ActionSheet items={["示例菜单", "示例菜单", "示例菜单", "示例菜单"]} onCancel={this.actionSheetOnCancel.bind(this)} onClick={this.actionSheetOnClick.bind(this)} /> : null;
        return (
            <div>
                <Button>按钮</Button>
                <hr />
                ...
                <div>
                    <div>Toast</div>
                    <Button onClick={this.showToast.bind(this)}>点击</Button>
                    {toast}
                </div>
                <hr />
                <div>
                    <div>Loading</div>
                    <Button onClick={this.loading.bind(this)}>点击</Button>
                    {loading}
                </div>
                <div>
                    <div>ActionSheet</div>
